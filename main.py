...
def parse_args():
    ...
    parser.add_argument('--bias_in_sa', type=str2bool, default=True, help='whether to use bias in the self-attention block')
    ...
    parser.add_argument('--da_policy', type=str, default='full', help='\'full\' to enable differentiable augmentation')
    ...
    parser.add_argument('--g_final_kernel', type=str, default="3", help='kernel size for convolutions in final layer, can specify multiple sizes, e.g. 32x3,32x5')
    parser.add_argument('--g_final_kernel_extra', type=str, default="3", help='kernel size for extra final layer, if enabled')
    ...
    parser.add_argument('--d_tex_recon_ch', type=int, default=96, help='base number of channels in discriminator texture reconstruction')
    parser.add_argument('--d_recon_ld', type=float, default=1.0, help='multiplicator for discriminator reconstruction loss')
    parser.add_argument('--d_tex_recon_ld', type=float, default=0.5, help='multiplicator for discriminator texture reconstruction loss')
    parser.add_argument('--d_recon_bn_after_act', type=str2bool, default=False, help='apply BN after activation in reconstruction layers')
    parser.add_argument('--d_save_recon_samples', type=str2bool, default=False, help='periodically save reconstructed samples')
    parser.add_argument('--d_final_conv', type=str2bool, default=False, help='use conv layer instead of pooling to reduce features to 1x1 in the discriminator')
